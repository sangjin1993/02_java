 ...
 *   ==> 매장에서 판매(출고)되어
 *   ==> 재고수량(quantity)이 amount 만큼 줄어들로록 반영
...
public class Product {
	...
	// 1. 메소드 해더가 동일 : 리턴타입 메소드이름(매개변수 목록)
	@Override
...
//	@Override
	 ...
	 * -----------------------------------------------------------------
	 * 1. 비교하려는 필드가 참조형이면 그 클래스가 재정의해둔
	 *    hashCode() 를 호출해서 비교
	 * 2. 비교하려는 필드가 기본형이면 기본형데이터를 참조형으로
	 * 	   변경하는 포장클래스 객체로 변환후 hashCode()를 적용
	 * 3. 비교하려는 필드가 1개 이상이면 1, 2를 각 필드의 타입에 맞추어
	 *    적용하고 모든 필드의 결과를 XOR(^) 연산을 통과시켜
	 *    그 결과를 리턴한다.
	 * ------------------------------------------------------------------
	 * @Override
	 * public int hashCode() {
